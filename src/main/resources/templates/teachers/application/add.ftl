...
<form class="layui-form" method="post" action="/promote/add" style="width: 80%;margin: 20px auto">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <select name="promoteId" lay-filter="" lay-verify="required">
                <#list promoteList as item>
                    <option value="${item.promoteId}">${item.name}</option>
                </#list>
            </select>
        </div>
    </div>
    ...
    layui.use(['form', 'layer'], function () {
        ...
        form.on('submit(*)', function (data) {  // *是提交按钮的lay-filter="*"
            ...
            $.ajax({
                ...
                dataType: 'json',
                data: data.field,
                url: "/application/apply/"+data.field.promoteId,
