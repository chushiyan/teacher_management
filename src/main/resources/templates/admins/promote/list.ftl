...
<script type="text/html" id="toolbar">
    <div class="layui-table-tool-temp" style="float: left;padding-right: 20px;">
        ...
        <input type="text" id="search_input" style="width:200px;float:right;height:30px;"
               placeholder="请输入职称名" autocomplete="off" class="layui-input">
    </div>
</script>
...
<script>
    layui.use(['layer', 'table'], function () {
        ...
        table.on('tool(test)', function (obj) {
            ...
            } else if (obj.event === 'edit') {  // 点击了编辑按钮
        ...
        table.on('edit(test)', function (obj) { //注：edit是固定事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            $.ajax({
                ...
                data: obj.data,
                url: "/promote/update/" + obj.data.promoteId,
                success: function (data) {
                    layer.msg(
                            data.message,
                            {
        ...
        table.on('toolbar(test)', function (obj) {
            switch (obj.event) {
                ...
                case "search": // 如果是点击了搜索按钮
                    ...
                    table.reload('test', {
                        // url可以不指定，会使用table.render中指定的
                        url: '${base}/promote/list'  // 因为设置了分页，所以会带上参数发请求 ?page=1&limit=5&query= （query是下面where中定义的参数名）
                        ,
                        // ,methods:"post"
                        where: {  //设定异步数据接口的额外参数
                            query: inputVal
