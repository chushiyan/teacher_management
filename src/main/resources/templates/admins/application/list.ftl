...
<script type="text/html" id="barDemo">
    ...
    <a class="layui-btn layui-btn-xs" lay-event="skip">打回</a>
...
<script type="text/html" id="toolbar">
    <div class="layui-table-tool-temp" style="float: left;padding-right: 20px;">
        ...
        <div class="layui-inline" id="search_btn" style="float:right;height:29px;" title="搜索"
             lay-event="search">
...
<script>
    layui.use(['layer', 'table'], function () {
        ...
        table.render({
            ...
            url: '${base}/application/list', // 请求地址
            toolbar: "#toolbar",
        ...
        table.on('tool(test)', function (obj) {
            ...
            if (obj.event === 'check') {  // 点击了

                $.ajax({
                    type: 'POST',
                    dataType: 'json',
                    // data: data.field,
                    url: "/application/check/" + data.applicationId,
                        ...
                        layer.msg(
                                ...
                                function () {
                                    ...
                                    table.reload('test', {
                                        ...
                                        url: '${base}/application/list',  // 因为设置了分页，所以会带上参数发请求 list-data2.json?page=1&limit=5&query= （query是下面where中定义的参数名）
                                        // ,methods:"post"
                                        where: {  //设定异步数据接口的额外参数
                                            // query: inputVal
                                        },
            ...
            // 点击了删除按钮
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    // data: data.field,
                    url: "/application/skip/" + data.applicationId,
                    success: function (data) {
                        layer.msg(
                                data.message,
                                {
        ...
        table.on('toolbar(test)', function (obj) {
            ...
            switch (obj.event) {
                ...
                case "search": // 如果是点击了搜索按钮
                    ...
                    table.reload('test', {
                        // url可以不指定，会使用table.render中指定的
                        url: '${base}/application/list'  // 因为设置了分页，所以会带上参数发请求 ?page=1&limit=5&query= （query是下面where中定义的参数名）
                        ,
                        // ,methods:"post"
                        where: {  //设定异步数据接口的额外参数
                            query: inputVal
